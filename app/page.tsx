...
type Trade = { id: string; tradeRef: string; symbol: string; side: 'BUY' | 'SELL'; lots: number; openPrice: number; closePrice: number; pnl: number; source: 'robot' | 'manual' }
...
export default function Home() {
  ...
  return <div className={dark ? 'app-shell' : 'app-shell light-shell'}>
    ...
    <main className="main-area"><header className="topbar"><button className="mobile-menu" onClick={() => setMobileNav(true)}><Menu /></button><div className="crumb"><span>Workspace</span><ChevronDown /><span className="slash">/</span><b>{active}</b></div><div className="top-actions"><div className="search"><Search /><input placeholder="Search anything..." /><kbd>⌘ K</kbd></div><button className="top-icon" onClick={() => go('Alerts')}><Bell /><i /></button><button className="top-icon" onClick={() => setDark(!dark)}>{dark ? <Sun /> : <Moon />}</button></div></header><div className="content">{header(active, `Welcome back, ${currentUser?.name ?? ''}. Monitoring ${account.label}.`)}{accountBar}{page}<footer><span>FOREX for better living · {account.label}</span><span>Last sync: just now <i className="footer-live" /></span></footer></div>{notice && <div className="toast"><Check />{notice}</div>}</main>
  </div>
}
