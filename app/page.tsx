...
import { Activity, AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, Bell, Bot, Check, ChevronDown, CircleHelp, Copy, CreditCard, Download, Edit3, LayoutDashboard, LineChart, Menu, MoreHorizontal, Moon, Pause, Pencil, Play, Plus, Search, Server, Settings, ShieldCheck, Sun, Trash2, TrendingUp, Users, Wallet, X, Zap } from 'lucide-react'

type Account = { id: string; label: string; broker: string; status: 'Active' | 'Paused'; equity: number; balance: number; pnl: number; trades: number; winRate: number; margin: number; vpsId: string | null }
...
type Trade = { id: string; tradeRef: string; symbol: string; side: 'BUY' | 'SELL'; lots: number; openPrice: number; closePrice: number; pnl: number }
...
export default function Home() {
  ...
  const submitAccountForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { account: created } = await api<{ account: Account }>('/api/accounts', { method: 'POST', body: JSON.stringify({ broker, accountNumber, vpsId, balance }) })
  ...
  return <div className={dark ? 'app-shell' : 'app-shell light-shell'}>
    <div className="profile-control"><button className="profile-trigger" aria-label="Open profile menu" aria-expanded={userMenu} onClick={() => setUserMenu(value => !value)}><span className="profile-avatar">{initials}</span><ChevronDown /></button>{userMenu && <div className="profile-popover" role="menu"><div className="profile-summary"><span className="profile-avatar large">{initials}</span><div><strong>{currentUser?.name}</strong><small>{currentUser?.email}</small><em>{currentUser?.role}</em></div></div><button role="menuitem" onClick={() => { setUserMenu(false); setActive('Settings') }}><Settings />Profile & settings</button><button role="menuitem" className="logout-action" onClick={() => { setUserMenu(false); logout() }}><X />Log out</button></div>}</div>
    {accountForm && <div className="modal-backdrop"><section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="add-account-title"><div className="modal-head"><div><div className="eyebrow"><CreditCard /> ACCOUNT MANAGEMENT</div><h2 id="add-account-title">Add trading account</h2><p>Connect an account and assign its execution VPS.</p></div><button className="icon-button" aria-label="Close account form" onClick={() => setAccountForm(false)}><X /></button></div><form className="vps-form" onSubmit={submitAccountForm}><div className="vps-form-grid"><label>Broker<input required name="broker" placeholder="Broker name" /></label><label>Account number<input required name="number" placeholder="12345678" /></label><label>VPS assignment<select name="vps" defaultValue={vpsItems[0]?.id}>{vpsItems.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></label><label>Initial balance<input required name="balance" type="number" defaultValue="25000" /></label></div><div className="vps-form-actions"><button type="button" className="outline-button" onClick={() => setAccountForm(false)}>Cancel</button><button type="submit" className="primary-button"><Plus />Add account</button></div></form></section></div>}
    ...
    <aside className={mobileNav ? 'sidebar mobile-open' : 'sidebar'}><div className="brand"><img src="/fbl-logo.png" alt="Forex For Better Living" style={{ height: 28, width: 'auto' }} /><button className="close-nav" onClick={() => setMobileNav(false)}><X /></button></div><div className="workspace"><div className="workspace-dot" /><div><small>WORKSPACE</small><b>Personal Fund</b></div><ChevronDown /></div><div className="nav-label">Monitor</div><nav>{nav.map(([label, Icon]) => <button key={label} className={active === label ? 'nav-item active' : 'nav-item'} onClick={() => go(label)}><Icon />{label}{label === 'Robot Control' && <span className="live-dot" />}</button>)}</nav><div className="nav-label settings-label">Manage</div><nav><button className={active === 'Alerts' ? 'nav-item active' : 'nav-item'} onClick={() => go('Alerts')}><Bell />Alerts <span className="badge-count">3</span></button>{isAdmin && <button className={active === 'Users' ? 'nav-item active' : 'nav-item'} onClick={() => go('Users')}><Users />User management</button>}<button className={active === 'Settings' ? 'nav-item active' : 'nav-item'} onClick={() => go('Settings')}><Settings />Settings</button></nav><div className="sidebar-bottom"><div className="support"><CircleHelp /><div><b>Need help?</b><small>Visit support center</small></div></div><div className="profile"><div className="avatar">{initials}</div><div><b>{currentUser?.name}</b><small>{currentUser?.role}</small></div><MoreHorizontal /></div></div></aside>
    <main className="main-area"><header className="topbar"><button className="mobile-menu" onClick={() => setMobileNav(true)}><Menu /></button><div className="crumb"><span>Workspace</span><ChevronDown /><span className="slash">/</span><b>{active}</b></div><div className="top-actions"><div className="search"><Search /><input placeholder="Search anything..." /><kbd>⌘ K</kbd></div><button className="top-icon" onClick={() => go('Alerts')}><Bell /><i /></button><button className="top-icon" onClick={() => setDark(!dark)}>{dark ? <Sun /> : <Moon />}</button><div className="user-pill"><span className="avatar">{initials}</span><ChevronDown /></div></div></header><div className="content">{header(active, `Welcome back, ${currentUser?.name ?? ''}. Monitoring ${account.label}.`)}{accountBar}{page}<footer><span>FOREX for better living · {account.label}</span><span>Last sync: just now <i className="footer-live" /></span></footer></div>{notice && <div className="toast"><Check />{notice}</div>}</main>
  </div>
}
