...
type Account = { id: string; label: string; broker: string; accountNumber: string; status: 'Active' | 'Paused'; equity: number; balance: number; pnl: number; trades: number; winRate: number; margin: number; vpsId: string | null }
...
type NotificationSettings = { fonnteToken: string; recipientPhone: string; notifyTradeClosed: boolean; notifyRobotOffline: boolean; notifyWithdrawal: boolean }
...
export default function Home() {
  ...
  const submitAccountForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingAccountId) {
      const { account: updated } = await api<{ account: Account }>(`/api/accounts/${editingAccountId}`, { method: 'PATCH', body: JSON.stringify({ broker, accountNumber, vpsId, balance }) })
  ...
  const submitNotificationForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(e.currentTarget)
    const body = {
      fonnteToken: String(form.get('fonnteToken') || ''),
      recipientPhone: String(form.get('recipientPhone') || ''),
      notifyTradeClosed: form.get('notifyTradeClosed') === 'on',
      notifyRobotOffline: form.get('notifyRobotOffline') === 'on',
      notifyWithdrawal: form.get('notifyWithdrawal') === 'on',
    }
