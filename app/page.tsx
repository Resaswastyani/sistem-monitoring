...
type Account = { id: string; label: string; broker: string; accountNumber: string; customerName: string | null; customerPhone: string | null; status: 'Active' | 'Paused'; equity: number; balance: number; pnl: number; trades: number; winRate: number; margin: number; vpsId: string | null }
...
export default function Home() {
  ...
  const submitAccountForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingAccountId) {
      const { account: updated } = await api<{ account: Account }>(`/api/accounts/${editingAccountId}`, { method: 'PATCH', body: JSON.stringify({ broker, accountNumber, customerName, customerPhone, vpsId, balance }) })
      setAccounts(as => as.map(a => a.id === editingAccountId ? updated : a))
      action('Account updated')
    } else {
      const { account: created } = await api<{ account: Account }>('/api/accounts', { method: 'POST', body: JSON.stringify({ broker, accountNumber, customerName, customerPhone, vpsId, balance }) })
  ...
  const page = active === 'Overview' ? overview
    ...
    : <section className="panel data-page"><div className="panel-head"><div><h2>Settings</h2><p>Workspace and monitoring preferences</p></div></div><div className="settings-form"><label>Workspace name<input defaultValue="Personal Fund" /></label><label>Default account<select value={accountId} onChange={e => setAccountId(e.target.value)}>{accounts.map(a => <option key={a.id} value={a.id}>{a.label}</option>)}</select></label><label className="switch-row"><span>Auto-pause on risk threshold</span><input type="checkbox" defaultChecked /></label><button className="primary-button" onClick={() => action('Settings saved')}><Check />Save settings</button>{isAdmin && <><label style={{ marginTop: 8 }}>Profit-sharing rules<span style={{ display: 'block', fontSize: 11, color: 'var(--dim)', fontWeight: 400 }}>Auto-calculated whenever a withdrawal is marked completed.</span></label>{profitRules.map(r => <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)' }}><span>{r.recipientName}</span><b>{r.percentage}%</b><button className="icon-button danger-button" aria-label="Remove rule" onClick={() => deleteRule(r.id)}><Trash2 /></button></div>)}<form className="vps-form-grid" onSubmit={submitRuleForm}><label>Recipient<input name="recipientName" required placeholder="Partner name" /></label><label>Percentage<input name="percentage" required type="number" min="0" max="100" step="0.1" placeholder="20" /></label><button type="submit" className="outline-button" style={{ alignSelf: 'end' }}><Plus />Add rule</button></form><label style={{ marginTop: 8 }}>WhatsApp gateway (Baileys)<span style={{ display: 'block', fontSize: 11, color: 'var(--dim)', fontWeight: 400 }}>URL &amp; API key dari WhatsApp gateway yang Anda jalankan sendiri (self-hosted).</span></label><form key={notifSettings ? 'loaded' : 'loading'} className="vps-form-grid" onSubmit={submitNotificationForm}><label>Gateway URL<input name="gatewayUrl" defaultValue={notifSettings?.gatewayUrl ?? ''} placeholder="https://your-gateway.example.com" /></label><label>Gateway API key<input name="gatewayApiKey" defaultValue={notifSettings?.gatewayApiKey ?? ''} placeholder="secret key" /></label><label>Nomor WA owner<input name="ownerPhone" defaultValue={notifSettings?.ownerPhone ?? ''} placeholder="6281234567890" /></label><button type="submit" className="outline-button" style={{ alignSelf: 'end' }}><Check />Save gateway settings</button></form><label style={{ marginTop: 8 }}>Notification rules<span style={{ display: 'block', fontSize: 11, color: 'var(--dim)', fontWeight: 400 }}>Pilih siapa yang dapat WA untuk tiap jenis kejadian.</span></label>{notifRules.map(r => <div key={r.eventType} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><b style={{ fontSize: 12 }}>{eventTypeLabels[r.eventType]}</b><label style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 11, color: 'var(--dim)' }}>Aktif</span><input type="checkbox" checked={r.active} onChange={e => updateNotifRule(r.eventType, { active: e.target.checked })} /></label></div><div style={{ display: 'flex', gap: 18, marginTop: 8 }}><label style={{ display: 'flex', alignItems: 'center', gap: 6 }}><input type="checkbox" checked={r.notifyOwner} onChange={e => updateNotifRule(r.eventType, { notifyOwner: e.target.checked })} /><span style={{ fontSize: 11, color: 'var(--dim)' }}>Notify owner</span></label><label style={{ display: 'flex', alignItems: 'center', gap: 6 }}><input type="checkbox" checked={r.notifyClient} onChange={e => updateNotifRule(r.eventType, { notifyClient: e.target.checked })} /><span style={{ fontSize: 11, color: 'var(--dim)' }}>Notify client</span></label></div></div>)}</>}</div></section>
